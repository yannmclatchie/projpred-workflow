...
run_loo_experiment <- function(N_sim,N,n_rel,n_irrel){
  ...
  for(i in 1:N_sim){
    ...
    if(i==1){
        ...
        prior_mod_r2d2 <- prior(R2D2(mean_R2=0.25,prec_R2=4)) + prior(exponential(1),class='sigma')
        mod_ref_r2d2 <- brm(ref_mod_formula,data=dat,prior=prior_mod_r2d2,family=gaussian(),refresh=0,chains=4,iter=4000,thin=2,cores=4,control=list(adapt_delta=0.9), save_pars = save_pars(all = TRUE))

        #reference model with normal prior on treatment and r2d2 prior on the rest
        mod_ref_r2d2normal<- brm(ref_mod_formula,data=dat,prior=prior_mod_r2d2,family=gaussian(),refresh=0,chains=0,iter=16000,thin=8,cores=4,control=list(adapt_delta=0.9), save_pars = save_pars(all = TRUE))
        mod_ref_standata <- make_standata(ref_mod_formula,data=dat,prior=prior_mod_r2d2)
        ### TODO: Update file 'stan/r2d2_normal.stan' so that it also uses `R2D2(mean_R2=0.25,prec_R2=4)`:
    ...
    loo_mod_ref_r2d2 <- loo(mod_ref_r2d2, moment_match = TRUE, reloo = TRUE)
    loo_mod_ref_r2d2normal <- loo(mod_ref_r2d2normal, moment_match = TRUE, reloo = TRUE) # TODO: check whether moment_match results in an error
